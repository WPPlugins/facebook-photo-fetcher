...
{ 
    ...
    {
        echo __("Sorry, this plugin currently supports only one Facebook gallery per page.", 'facebook-photo-fetcher') . "<br />";
        return 0;
    }
    $retVal = Array();
    ...
    if( count($result) < 3 )
    {
        echo __("Missing gallery end-tag.", 'facebook-photo-fetcher') . "<br />";
        return 0;
    }
    if( count($result) > 3 )
    {
        echo __("Duplicate gallery end-tag found.", 'facebook-photo-fetcher') . "<br />";
        return 0;
    }
...
function fpf_fetch_album_content($aid, $params)
{
	...
	$debugPhotoCount = 0;
	$fetch_url = "https://graph.facebook.com/$fpf_apiver/$aid/photos?access_token=$access_token&limit=9999&fields=name,source,picture";
	while(true)
	{
