...
function fpf_admin_page()
{
    ...
    if( isset($_POST[$fpf_opt_access_token]) )  //User connected a facebook session (login+save)
    {
        ...
        if( isset($user->id) && !isset($user->error) )
        {
            ...
            ?><div class="updated"><p><strong><?php echo sprintf(__('Facebook Session Saved (Name: %s, ID: %s)', 'facebook-photo-fetcher'), $user->name, $user->id) ?></strong></p></div><?php
        }
        else
        {
            update_option( $fpf_opt_access_token, 0 );
            update_option( $fpf_opt_token_expiration, 0 );
            ?><div class="updated"><p><strong><?php echo __('Error: Failed to get a valid access token from Facebook.  Response:', 'facebook-photo-fetcher') . ' ' . (isset($user->error->message)?$user->error->message:"Unknown");?></strong></p></div><?php
        }    
    }
    ...
    else if( isset($_POST[$fpf_opt_last_uid_search]) )    //User clicked "Search," which saves 'last searched uid'
    {
        ...
        ?><div class="updated"><p><strong><?php _e('Album search completed.', 'facebook-photo-fetcher') ?></strong></p></div><?php
    }
    ...
    if(!$access_token)
    {
        ?><div class="error"><p><strong><?php _e('This plugin does not have a valid Facebook access token.  Please authorize it by logging in below.', 'facebook-photo-fetcher')?></strong></p></div><?php        
    }
    else if(!$user)
    {
        ?><div class="error"><p><strong><?php _e('An error occurred while validating your Facebook access token (empty reply).  Please re-authorize by logging in below.', 'facebook-photo-fetcher')?></strong></p></div><?php
        update_option($fpf_opt_access_token, 0);
    }
    else if(isset($user->error))
    {
        ?><div class="error"><p><strong><?php echo $user->error->message . "<br /><br />" . __('Please re-authorize this plugin by logging into Facebook below.', 'facebook-photo-fetcher')?></strong></p></div><?php
        update_option($fpf_opt_access_token, 0);
    }
    ...
    <div class="fpf-admin_wrapper">
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tab1Id?>" style="display:<?php echo ($fpf_shown_tab==1?"block":"none")?>">
            ...
            <?php _e('To get started, you must first connect with your Facebook account using the button below.  Once connected, you can create a gallery by making a new Wordpress post or page and pasting in one line of special HTML, like this:', 'facebook-photo-fetcher')?><br /><br />
            <b>&lt;!--<?php echo $fpf_identifier?> 1234567890123456789 --&gt;&lt;!--/<?php echo $fpf_identifier?>--&gt;</b><br /><br />
            <?php _e('Whenever you save a post or page containing these tags, this plugin will automatically download the album information and insert its contents between them.  You are free to include any normal content you like before or after, as usual.', 'facebook-photo-fetcher')?><br /><br />
            ...
            <?php echo sprintf(__("And if you like this plugin, please don't forget to %s donate %s a few bucks to buy me a beer (or a pitcher).  I promise to enjoy every ounce of it :)", 'facebook-photo-fetcher'), '<a href="javascript:void(0);" onclick="fpf_swap_tabs(\''.$tab4Id.'\');jQuery(\'html, body\').animate({ scrollTop: jQuery(document).height() }, \'slow\');">', "</a>")?><br /><br />
            <hr />
            ...
            <?php else:                ?> <?php _e('Before this plugin can be used, you must connect it to your Facebook account.  Please click the following button to login:', 'facebook-photo-fetcher')?><br /><br />
            <?php endif; ?>
            	...
            	<div id="graph_step1" style="width:150px;height:23px;float:left;">
            		...
            		<script>
            			var socket = new easyXDM.Socket(
            			{
            	    		...
            	    		remote: "<?php echo is_ssl()?"https":"http"; ?>://www.justin-klein.com/auth/FPF-Auth/",
            	    		
                            //Once it's ready, send a message to tell it what to name the login button & which plugin version we're using
                            onReady: function()
                            {
                                var message = {btnName:'<?php echo ($access_token?__("Renew", 'facebook-photo-fetcher'):__("Login with Facebook", 'facebook-photo-fetcher'))?>',
        ...
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tab2Id?>" style="display:<?php echo ($fpf_shown_tab==2?"block":"none")?>">    
           ...
           <form name="listalbums" method="post" action="">
               ...
               <?php echo sprintf(__("Your User ID is %s.  To get a page's ID, view any of its photos on Facebook - the URL will be something like %s. The last set of numbers (%s in this example) is the ID.", 'facebook-photo-fetcher'), "<b>$user->id</b>", "<b>facebook.com/photos/a.123.456.789</b>", "789")?><br /><br /> 
